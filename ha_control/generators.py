...
service_tmpl = "return self.call_service({service_name}, **kwargs)"
...
def generate_service_method(domain_name, service_name, service_data, indent_level=1):
    ...
    method_lines = [
        f'\n{" " * def_indent}@service_call("{domain_name}")',
...
def generate_domain_module(domains):
...
def write_domain_module(domains, module_path):
    domain_module = generate_domain_module(domains)
